...
- **事件**：撳 log 一行會跳去嗰位員工
- **關閉選取**：`Esc`
- inspector 入面可選「模擬對話」：靜態站唔會 Call 真 LLM，用離線模擬
...
```bash
# 方法 B：沿用原本 Express（可選；純瀏覽用方法 A 就得）
npm install
npm start
...
如果已經接咗 Vercel：前端靜態檔照常睇指揮中心；`GET /api/health` 用嚟探活。inspector 入面嘅可選對話係離線模擬。**純瀏覽唔需要 API。**
